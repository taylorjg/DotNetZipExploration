...
namespace DotNetZipExploration
{
    ...
    internal class DotNetZipTests
    {
        ...
        [Test]
        public void
            GivenAStreamContainingAZipFileContainingAHierarchyOfFilesAndSubDirectories_WeCanCreateADictionaryOfSubDirectoriesToFiles
            ()
        {
            // Arrange
            using (var zipFileStream = ReadZipFile("Hierarchy.zip"))
            {
                ...
                {
                    var lastSlash = e.FileName.LastIndexOf('/');
                    var subDirectory = lastSlash >= 0 ? e.FileName.Substring(0, lastSlash + 1) : string.Empty;
                    var fileName = lastSlash >= 0 ? e.FileName.Substring(lastSlash + 1) : e.FileName;
